...
**14A.2 列宽拖动**：表头右缘 1.5px 手柄，hover 显紫色；拖动最小列宽 50px；表格 `table-layout: fixed` + 像素宽度。**无障碍强化**：手柄加 `role="separator"` + `aria-valuenow`（当前宽）+ `tabindex="0"`，左右方向键 ±10px（Shift+方向键 ±1px 精调）。
...
*v5.1 · 2026-09-12 · 依据 ALE Corporate Brand Guidelines（September 2025）、v4.1/v5.0 与三个实现参照仓库源码实测整理*
