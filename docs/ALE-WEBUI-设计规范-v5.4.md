...
| **v5.4.1** | 2026-09-12 | **勘误（专家复核 DONE_WITH_CONCERNS 80/100 后收口）**：①修复字体资产 404——deploy 打包清单补 `fonts` 与 version.json（P0-A，同时暴露 fonts.check 系统字体假阳性，资产 200 检查纳入发布门禁）；②演示站/Hub 同步二维模型，移除“三种产品形态”表述（P0-B）；③`.principle h3` 残留 .95rem 统一 1rem（P0-C）；④三份汇报材料 v5.2 残留清理、P0 编号统一为 P0-1 至 P0-5（P1-A/B）。v5.4.1 为只读基线，v6.0 治理版按评审八周路线另启。 |
