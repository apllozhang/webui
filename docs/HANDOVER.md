...
**重要里程碑 tag**：`v5.4.1`（只读基线）；**版本真源 2026-09-15 正式为 `6.0.0`**（v6.0.0 tag 发布；rc 签收基线 349e02d，发布基线见 release-lock.json）。
...
- **安全决策记录（负责人已拍板，选 A；约束②已于 M5-0.2 执行；M5-0.3 曾轮换、同日负责人裁决测试阶段回退为原口令——暴露风险已知悉，建议 M6 正式发布前再轮换）**
